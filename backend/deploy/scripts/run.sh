#!/usr/bin/env bash
set -euo pipefail

# Usage:
#   ./deploy/scripts/run.sh prod
#   ENVIRONMENT=dev ./deploy/scripts/run.sh
ENVIRONMENT="${1:-${ENVIRONMENT:-prod}}"

if [[ "$ENVIRONMENT" != "dev" && "$ENVIRONMENT" != "prod" ]]; then
  echo "[ERROR] ENVIRONMENT must be 'dev' or 'prod' (got: $ENVIRONMENT)"
  exit 1
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"

BASE_COMPOSE_FILE="$PROJECT_ROOT/deploy/compose/compose.yml"
ENV_COMPOSE_FILE="$PROJECT_ROOT/deploy/compose/compose.${ENVIRONMENT}.yml"
ENV_FILE="$PROJECT_ROOT/deploy/env/.env.${ENVIRONMENT}"

if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then
  COMPOSE_CMD=(docker compose)
elif command -v docker-compose >/dev/null 2>&1; then
  COMPOSE_CMD=(docker-compose)
else
  echo "[ERROR] docker compose(v2) or docker-compose(v1) not found"
  exit 1
fi

for f in "$BASE_COMPOSE_FILE" "$ENV_COMPOSE_FILE" "$ENV_FILE"; do
  if [[ ! -f "$f" ]]; then
    echo "[ERROR] Missing file: $f"
    exit 1
  fi
done

cd "$PROJECT_ROOT"

if [[ ! -f "./gradlew" ]]; then
  echo "[ERROR] ./gradlew not found in project root: $PROJECT_ROOT"
  exit 1
fi
if [[ ! -x "./gradlew" ]]; then
  chmod +x ./gradlew
fi

echo "[STEP] Build Spring Boot jar"
./gradlew clean build -x test

echo "[STEP] Stop old containers"
"${COMPOSE_CMD[@]}" --env-file "$ENV_FILE" \
  -f "$BASE_COMPOSE_FILE" -f "$ENV_COMPOSE_FILE" down --remove-orphans

echo "[STEP] Pull latest backend image"
"${COMPOSE_CMD[@]}" --env-file "$ENV_FILE" \
  -f "$BASE_COMPOSE_FILE" -f "$ENV_COMPOSE_FILE" pull backend || true

echo "[STEP] Start services"
"${COMPOSE_CMD[@]}" --env-file "$ENV_FILE" \
  -f "$BASE_COMPOSE_FILE" -f "$ENV_COMPOSE_FILE" up -d --build

echo "[DONE] Deploy finished for '$ENVIRONMENT'"
"${COMPOSE_CMD[@]}" --env-file "$ENV_FILE" \
  -f "$BASE_COMPOSE_FILE" -f "$ENV_COMPOSE_FILE" ps
