...
public class CustomMemberDetailsService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String socialId) {
		...
		// 2. 회원 조회
		MatchingProfile matchingProfile = findMemberBySocialId(socialId);
		// 3. CustomMemberDetails 생성 및 반환
		return new CustomMemberDetails(matchingProfile);
	}
	 ...
	 */
	private MatchingProfile findMemberBySocialId(String socialId) {
		return matchingProfileRepository.findBySocialId(socialId.trim())
